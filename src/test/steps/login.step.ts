...
Given('el usuario se encuentra en el login de SauceDemo', async function () {
    await pageFixture.page.goto(process.env.URL);
    await pageFixture.page.waitForLoadState('networkidle');
});

When('ingresa un nombre de usuario válido y contraseña válida', async function () {
...
When('ingresa un nombre de usuario inválido o contraseña inválida', async function () {
    const credentials=new LoginPage(pageFixture.page);
    await credentials.fillUsername('locked_out_user');
    await credentials.fillPassword('secret_sauce');
});

Then('debe mostrarse un mensaje de error indicando que las credenciales son incorrectas', async function () {
